...
func _save_data():
	...
	file.store_line(JSON.new().stringify(player_list))
	file.close()

func _create_data():
	var file = File.new()
	file.open(SAVE_FILE, File.WRITE)
	file.store_line(JSON.new().stringify(player_list_default))
	file.close()
...
func _load_data():
	...
	player_list = test_json_conv.get_data()
